...
- v1.0.5 (2026-04-28)
  ...
  - asmdef 名および Root Namespace の例を `Uraty.Features.*` / `Uraty.Systems.*` に更新
- v1.0.4 (2026-03-19)
  - `bool` の命名規則を削除
- v1.0.3 (2026-03-07)
  - ディレクトリ名の規約を変更
...
- **Application**: 起動・合成（composition root）・ゲーム全体の流れを司る、`Feature` / `Shared` を束ねる制御側。
- **System**: 入力、保存、通信、アセット読み込み等の基盤機能。
- **asmdef（Assembly Definition）**: Unity における C# スクリプトのコンパイル単位を定義するアセット。アセンブリ境界と依存方向を明示するために使用する。
...
### asmdef
- `_Application` は `Uraty.Application.asmdef` を配置する
- `_Shared` は `Uraty.Shared.asmdef` を配置する
- `_Features` 直下に共通の asmdef は置かず、各 `Feature` 単位で asmdef を配置する
- `_Systems` 直下に共通の asmdef は置かず、各 `System` 単位で asmdef を配置する
- `Feature` の asmdef 名は `Uraty.Features.<FeatureName>` とする
- `System` の asmdef 名は `Uraty.Systems.<SystemName>` とする
- `Application` は実際に利用する `Feature` / `System` の asmdef のみを個別に参照する
- `Feature` 間で直接参照しない
- `Shared` は `Application` / `Feature` / `System` に依存しない
...
### 基本方針
「Feature（機能）縦割り」＋「Application / Shared / System」

- `Feature` を第一の配置基準とする
- `Shared` は再利用実績または再利用計画が明確なものに限定する
- `Application` は起動・合成（composition root）・ゲーム全体の流れ・複数 `Feature` の仲介を担う
- `System` はアプリ全体で利用される横断的なシステム機能を置く
...
### 依存ルール
- MAY:
  - `Application` は `Feature` に依存してよい
  - `Application` は `Shared` に依存してよい
  - `Application` は `System` に依存してよい
  - `Feature` は `Shared` に依存してよい
  - `Feature` は `System` に依存してよい
  - `System` は `Shared` に依存してよい
- MUST NOT:
  - `Feature` は `Application` に依存してはならない
  - `Feature` は他の `Feature` に依存してはならない
  - `Shared` は `Application` に依存してはならない
  - `Shared` は `Feature` に依存してはならない
  - `Shared` は `System` に依存してはならない
  - `System` は `Application` に依存してはならない
  - `System` は `Feature` に依存してはならない
- SHOULD NOT:
  - `Application` は `Feature` の内部実装に直接依存しない
  - `Feature` は `System` の具体実装に過剰に依存しない
  - `Shared` に特定の `Feature` 専用概念を持ち込まない

### 補足ルール
- 複数 `Feature` の連携は `Application` が仲介する
- 共通化したい処理があっても、再利用実績または再利用計画が明確でない限り `Shared` に置かない
- 依存は循環させない
- 配置に迷った場合は、まず `Feature` に置き、必要になった時点で `Shared` へ昇格する
...
- ルール:
  - MUST:
    ...
    - 各 `Feature` 直下は以下の標準フォルダ名から作成しなければならない
      - `Scenes/`
      - `Prefabs/`
      - `Scripts/`
      - `UI/`
...
### Application
- 目的: 起動・合成（composition root）・ゲーム全体の流れ・複数 `Feature` の仲介を行う
- ルール:
  - MUST:
    - 起動処理、初期化順序の制御、シーン遷移、ゲーム全体フロー、複数 `Feature` の連携を配置しなければならない
    - 複数 `Feature` の連携が必要な場合は `Application` が仲介しなければならない
  - MUST NOT:
    - `Input`、`Save`、`Audio`、通信、アセット読み込み等の基盤実装そのものを `Application` に直接配置してはならない
    - 特定 `Feature` の内部実装に直接依存してはならない

### System
- 目的: アプリ全体で利用される横断的な基盤機能を格納する
- ルール:
  - MUST:
    - 入力、保存、通信、アセット読み込み、音声、時刻、端末依存処理など、特定 `Feature` に属さない基盤機能を配置しなければならない
  - MUST NOT:
    - 特定 `Feature` 専用の概念や業務ルールを配置してはならない
    - `Feature` に依存してはならない
...
### 目的
コンパイル境界を明確化し、レイヤ間の依存方向を Unity のアセンブリ参照で強制する。

### 基本方針
- asmdef は「整理のため」ではなく「依存方向を強制するため」に使用する
- 存在していない責務の asmdef は先回りして作成しない
- 大きすぎる共通 asmdef を作って依存境界を曖昧にしない

### 配置ルール
- MUST:
  - `_Application` には `Uraty.Application.asmdef` を配置しなければならない
  - `_Shared` には `Uraty.Shared.asmdef` を配置しなければならない
  - `_Features` 配下の asmdef は `Feature` 単位で配置しなければならない
  - `_Systems` 配下の asmdef は `System` 単位で配置しなければならない
- MUST NOT:
  - `_Features` 直下に全 `Feature` をまとめる単一 asmdef を配置してはならない
  - `_Systems` 直下に全 `System` をまとめる単一 asmdef を配置してはならない
- SHOULD:
  - asmdef は責務のルートフォルダに配置する
  - `Feature` / `System` の分割単位が明確になった時点で作成する

### 命名
- MUST:
  - asmdef 名は `Uraty` をルートとして、名前空間規約と整合する名前にしなければならない
  - `_Features` 配下の asmdef 名は `Uraty.Features.<FeatureName>` 形式にしなければならない
  - `_Systems` 配下の asmdef 名は `Uraty.Systems.<SystemName>` 形式にしなければならない
  - `Uraty.Feature.*` 形式の asmdef 名を使用してはならない
  - `Uraty.System.*` 形式の asmdef 名を使用してはならない
- SHOULD:
  - asmdef 名、ファイル名、`Root Namespace` は一致させる
- 例:
  - OK:
    - `Uraty.Application`
    - `Uraty.Shared`
    - `Uraty.Features.Player`
    - `Uraty.Features.Bullet`
    - `Uraty.Systems.Input`
    - `Uraty.Systems.Audio`
  - NG:
    - `Application`
...
### 参照ルール
- MUST:
  - `Application` が `Shared` を利用する場合、`Uraty.Application.asmdef` から `Uraty.Shared` を参照しなければならない
  - `Application` が `Feature` を利用する場合、利用する `Uraty.Features.*` の asmdef のみを個別に参照しなければならない
  - `Application` が `System` を利用する場合、利用する `Uraty.Systems.*` の asmdef のみを個別に参照しなければならない
  - `Feature` が `Shared` を利用する場合、`Uraty.Shared` を参照しなければならない
  - `Feature` が `System` を利用する場合、利用する `Uraty.Systems.*` の asmdef のみを個別に参照しなければならない
  - `System` が `Shared` を利用する場合、`Uraty.Shared` を参照しなければならない
- MUST NOT:
  - `Feature` は他の `Feature` の asmdef を参照してはならない
  - `Feature` は `Application` の asmdef を参照してはならない
  - `Shared` は `Application` / `Feature` / `System` の asmdef を参照してはならない
  - `System` は `Application` / `Feature` の asmdef を参照してはならない
- SHOULD:
  - 参照は名前ではなく GUID で管理する
- SHOULD NOT:
  - 実際に利用していない asmdef への参照を追加しない

### 運用ルール
- MUST:
  - `Application` は実際に利用する `Feature` / `System` の asmdef のみを追加しなければならない
- SHOULD:
  - 新しい `Feature` / `System` を追加する際は、該当フォルダ作成と同時に asmdef を作成する
- SHOULD NOT:
  - 「将来使うかもしれない」を理由に参照を先回り追加しない

### 補足
- `Application` が最終的に多くの `Feature` を参照すること自体は問題ではない
- 問題なのは、`Feature` を1つの巨大 asmdef にまとめて境界を消すことである
- `Shared` は依存のハブにしてはならない
...
- asmdef 名と `Root Namespace` の整合
- asmdef 名が `Uraty.Features.*` / `Uraty.Systems.*` 形式になっているか
- `Uraty.Feature.*` / `Uraty.System.*` 形式の asmdef 名が残っていないか
- `_Application` / `_Shared` の asmdef 存在確認
- `_Features` / `_Systems` 直下の単一巨大 asmdef の禁止
- `Feature` → `Feature` の asmdef 参照禁止
- `Shared` → `Application` / `Feature` / `System` の asmdef 参照禁止
- `System` → `Application` / `Feature` の asmdef 参照禁止