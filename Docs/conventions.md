...
- MUST:
  ...
  - `Application` が `Feature` を利用する場合、利用する `Feature` の asmdef のみを個別に参照しなければならない
  - `Application` が `System` を利用する場合、利用する `System` の asmdef のみを個別に参照しなければならない
  - `Feature` が `Shared` を利用する場合、`Uraty.Shared` を参照しなければならない
  - `Feature` が `System` を利用する場合、利用する `System` の asmdef のみを個別に参照しなければならない
